...
Controles:
  Jugador 1 : MOUSE (mover el cursor para dirigir)
  Jugador 2 : W A S D
  Jugador 3 : Flechas
  Jugador 4 : T F G H
  ESC / P   : Pausar
...
# Cada entrada: (nombre, color_cabeza, color_cuerpo)
AVAILABLE_COLORS = [
    ...
    # Banderas
    ("🇫🇷 Francia",  (0, 85, 164),    (237, 41, 57)),   # azul/rojo francés
    ("🇦🇷 Argentina",(116, 185, 232),  (255, 255, 255)), # celeste/blanco
    ("🇵🇹 Portugal", (0, 102, 0),     (255, 0, 0)),      # verde/rojo
    ("🇧🇷 Brasil",   (0, 156, 59),    (255, 223, 0)),    # verde/amarillo
    ("🇪🇸 España",   (170, 21, 27),   (241, 191, 0)),    # rojo/dorado
    ("🇬🇧 Inglaterra",(255, 255, 255), (200, 16, 46)),   # blanco/rojo
]

# Colores por defecto para los 4 slots de jugadores
DEFAULT_COLOR_INDICES = [0, 6, 8, 4]  # Verde, Celeste, Rojo, Amarillo

FOOD_COLORS = [
    (255,45,120),(0,255,136),(0,200,255),(255,220,0),(255,120,0),(180,80,255),
]

# Jugador 1 ahora usa mouse; los demás desplazan un índice
CONTROLS = [
...
class Snake:
    def __init__(self, idx, is_ai, x, y, angle, custom_name=None, color_entry=None):
        ...
        self.segments = [
            [x - math.cos(angle)*i*RADIUS_BASE*1.8,
             y - math.sin(angle)*i*RADIUS_BASE*1.8]
            for i in range(INITIAL_SEGMENTS)
        ]

        # Control por mouse solo para jugador 0 humano
    ...
    def update(self, keys, snakes, food_list, cam_x, cam_y, viewport):
        ...
        if self.is_ai:
            ...
            # idx desplazado para teclado (idx 1 → CONTROLS[0], idx 2 → CONTROLS[1], etc.)
            ctrl_idx = self.idx - 1
    ...
    def _mouse_control(self, cam_x, cam_y, viewport):
        """Apuntar hacia el cursor del mouse en coordenadas de mundo."""
    ...
    def draw(self, surf, cam_x, cam_y):
        ...
        if self.segments:
            ...
            name_surf = FONT_XS.render(self.color["name"], True, head_c)
...
def render_viewport(surf, vp, cam_x, cam_y, snakes, food_list, particles, owner):
    ...
    pygame.draw.rect(sub,(2,5,7),(4,4,200,50),border_radius=6)
    pygame.draw.rect(sub,col,   (4,4,200,50),1,border_radius=6)
    ...
    # Cursor de mouse para jugador 1
    if owner.use_mouse and owner.alive:
        mx, my = pygame.mouse.get_pos()
        rel_x = mx - vp.x
        rel_y = my - vp.y
        if 0 <= rel_x < vw and 0 <= rel_y < vh:
            pygame.draw.circle(sub, owner.color["head"], (rel_x, rel_y), 6, 2)
            pygame.draw.line(sub, owner.color["head"],
                             (rel_x-10, rel_y), (rel_x+10, rel_y), 1)
            pygame.draw.line(sub, owner.color["head"],
                             (rel_x, rel_y-10), (rel_x, rel_y+10), 1)
...
def kill_snake(sn, food_list, particles):
    ...
    sn.alive = False
    play(SFX_DIE)
    hx, hy = sn.head  # guardar posición antes de limpiar
    # Spawnear comida en donde estaban los segmentos
    for i in range(0, len(sn.segments), 4):
        seg = sn.segments[i]
        food_list.append({"x": seg[0], "y": seg[1],
                          "r": random.uniform(6, 10), "color": sn.color["head"],
                          "pulse": 0, "value": 2})
    # Limpiar segmentos INMEDIATAMENTE para eliminar la hitbox
    sn.segments = []
...
def run_game(num_humans, use_ai, player_names, player_colors):
    ...
    # Ocultar cursor del sistema mientras se juega
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type==pygame.KEYDOWN:
                if event.key in (pygame.K_ESCAPE,pygame.K_p): paused=not paused
        ...
        for i, sn in enumerate(snakes):
            vp = vps[i] if (not sn.is_ai and i < len(vps)) else None
            cam = cams[i] if (not sn.is_ai and i < len(cams)) else ([sn.segments[0][0], sn.segments[0][1]] if sn.segments else [0, 0])
            sn.update(keys, snakes, food_list, cam[0], cam[1], vp)
        ...
        for sn in snakes:
            ...
            for other in snakes:
                # BUG FIX: ignorar serpientes muertas (sin segmentos)
                if not other.alive or not other.segments: continue
                skip=10 if other is sn else 0; hit=False
                for seg in other.segments[skip:]:
                    if dist(hx,hy,seg[0],seg[1])<sn.radius+other.radius*0.85:
        ...
        for i,sn in enumerate(humans):
            if sn.alive and sn.segments:
                tx, ty = sn.head
            else:
                tx, ty = cams[i]
            cams[i][0]+=(tx-cams[i][0])*0.08
            cams[i][1]+=(ty-cams[i][1])*0.08

        screen.fill((0,0,0))
        for i,(sn,vp) in enumerate(zip(humans,vps)):
            render_viewport(screen,vp,cams[i][0],cams[i][1],snakes,food_list,particles,sn)
...
def menu_screen():
    ...
    while True:
        ...
        ctrl_info = [
            ("J1", "MOUSE",   AVAILABLE_COLORS[DEFAULT_COLOR_INDICES[0]][1]),
            ("J2", "WASD",    AVAILABLE_COLORS[DEFAULT_COLOR_INDICES[1]][1]),
            ("J3", "FLECHAS", AVAILABLE_COLORS[DEFAULT_COLOR_INDICES[2]][1]),
            ("J4", "T/F/G/H", AVAILABLE_COLORS[DEFAULT_COLOR_INDICES[3]][1]),
        ]
        cy0=sh//2+90; cx0_c=sw//2-(4*160+3*12)//2
        for i,(jid,ks,col) in enumerate(ctrl_info):
            rx=cx0_c+i*172
            pygame.draw.rect(screen,(0,15,8),(rx,cy0,160,52),border_radius=6)
            pygame.draw.rect(screen,(*col,120),(rx,cy0,160,52),1,border_radius=6)
...
def nickname_screen(num_humans):
    ...
    while True:
        ...
        draw_text_centered(screen, "NOMBRE Y COLOR", FONT_BIG, (0,255,136), sw//2, 60)
        draw_text_centered(screen, "Solo letras y números  |  3 a 15 caracteres",
                           FONT_XS, (0,150,80), sw//2, 110)
        ...
        for i in range(num_humans):
            ...
            cname, chead, cbody = AVAILABLE_COLORS[color_indices[i]]
            px_start = fx + 350
            py_center = fy + 36
            for si in range(8):
                px = px_start + si * 14
            ...
            cname_surf = FONT_XS.render(cname, True, chead)
        ...
        for i in range(1, num_humans):
            sep_y = start_y + i * SECTION_H - 8
            pygame.draw.line(screen, (0,60,30), (sw//2-340, sep_y), (sw//2+340, sep_y), 1)

        cont_rect = pygame.Rect(sw//2 - 140, sh - 80, 280, 56)
        pygame.draw.rect(screen, (0,200,80), cont_rect, border_radius=10)
        draw_text_centered(screen, "CONTINUAR", FONT_MED, (2,5,2), sw//2, sh - 52)
        draw_text_centered(screen, "TAB para cambiar campo  |  ESC para volver",
                           FONT_XS, (0,100,50), sw//2, sh - 18)
